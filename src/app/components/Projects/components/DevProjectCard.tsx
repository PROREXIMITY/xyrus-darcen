...
import type { Project } from "@/app/components/data/mockData";
...
interface DevProjectCardProps {
  ...
  openCarousel: (images: string[], startIndex?: number) => void;
}
...
export default function DevProjectCard({
  ...
  const prefersCarousel =
    ...
    (!project.liveLink || project.liveLink.trim() === "" || project.liveLink.includes("github.com"));

  return (
    <div
      className={`flex ${isEven ? "justify-start" : "justify-end"} px-0 md:px-4 transition-all duration-500 ${
        isBlurred ? "blur-sm scale-[0.98] opacity-70" : ""
      }`}
      onMouseEnter={() => setHoveredIndex(index)}
      onMouseLeave={() => setHoveredIndex(null)}
    >
      <Tilt className="w-full lg:w-[900px]" tiltMaxAngleX={2} tiltMaxAngleY={2} scale={1.01} transitionSpeed={2000} gyroscope={false}>
        <div className="relative group/card">
          ...
          <div className="relative bg-[rgba(20,20,25,0.95)] backdrop-blur-xl rounded-2xl transition-all duration-500 overflow-hidden hover:shadow-2xl hover:shadow-[rgb(200,80,192)]/20 border border-white/10 group-hover/card:border-white/20">
            ...
            <div className={`flex flex-col ${isEven ? "md:flex-row" : "md:flex-row-reverse"} relative z-10`}>
              ...
              <div
                className={`flex-[35] bg-white/5 p-4 md:p-8 flex flex-col justify-center gap-3 md:gap-4 border-t md:border-t-0 ${
              ...
              >
                {prefersCarousel ? (
                  <button
                    ...
                    onClick={() =>
                      openCarousel(
                        project.screenshots && project.screenshots.length ? project.screenshots : [project.image],
                        0,
                      )
                    }
                  ...
                  >
                    <div className={`relative overflow-hidden flex items-center justify-between p-4 bg-gradient-to-r from-[rgb(65,88,208)] via-[rgb(200,80,192)] to-[rgb(255,204,112)] rounded-xl ${styles.gradientX} ${styles.hoverGlow}`}>
                      ...
                      <svg className="w-5 h-5 text-white transform group-hover/primary:translate-x-1 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <a href={project.liveLink} target="_blank" rel="noopener noreferrer" className="group/primary block">
                    <div className={`relative overflow-hidden flex items-center justify-between p-4 bg-gradient-to-r from-[rgb(65,88,208)] via-[rgb(200,80,192)] to-[rgb(255,204,112)] rounded-xl ${styles.gradientX} ${styles.hoverGlow}`}>
                      <span className="text-white font-bold">View Project</span>
                      <svg className="w-5 h-5 text-white transform group-hover/primary:translate-x-1 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                ...
                {project.githubLink ? (
                  <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="group/secondary block">
                    <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10 hover:border-white/30 hover:bg-white/10 transition-all duration-300 hover:scale-[1.02]">
                      <span className="text-white/90 font-medium">View Code</span>
                      <svg className="w-5 h-5 text-white/90 transform group-hover/secondary:translate-x-1 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="p-4 bg-gradient-to-r from-amber-500/10 to-orange-500/10 rounded-xl border border-amber-500/30 backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="text-amber-200 font-semibold text-sm">Private Repository</div>
                        <div className="text-amber-300/70 text-xs">Client project code</div>
                      </div>
                    </div>
                  </div>
                )}
